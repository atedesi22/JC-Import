          ...
          <div className="min-h-screen bg-slate-950 text-white font-sans selection:bg-amber-500/30 selection:text-amber-300">
            ...
            <nav className="fixed top-0 left-0 right-0 h-16 bg-slate-950/80 backdrop-blur-md border-b border-slate-900 z-50 px-4 md:px-8">
              <div className="max-w-7xl mx-auto h-full flex items-center justify-between">
                ...
                >
                  <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-amber-400 to-amber-600 flex items-center justify-center text-slate-950 font-black text-sm shadow-md shadow-amber-500/10 transition-transform group-hover:scale-105">
                    JC
                  </div>
                  <span className="font-bold tracking-wider text-sm md:text-base group-hover:text-amber-400 transition-colors">
                    IMPORT
                  </span>
                </div>

                {/* MENUS (Adaptés aux pouces sur mobile) */}
                <div className="flex items-center gap-1 sm:gap-4">
                  <button 
                    onClick={() => setCurrentPage('home')}
                    className={`px-3 py-1.5 rounded-lg text-xs md:text-sm font-medium transition-colors ${currentPage === 'home' ? 'text-amber-400 bg-slate-900' : 'text-gray-400 hover:text-white'}`}
                  >
                    Accueil
                  </button>
                  <button 
                    onClick={() => setCurrentPage('catalog')}
                    className={`px-3 py-1.5 rounded-lg text-xs md:text-sm font-medium transition-colors ${currentPage === 'catalog' ? 'text-amber-400 bg-slate-900' : 'text-gray-400 hover:text-white'}`}
                  >
                    Catalogue
                  </button>
                  <button 
                    onClick={() => setCurrentPage('groupage')}
                    className={`px-3 py-1.5 rounded-lg text-xs md:text-sm font-medium transition-colors ${currentPage === 'groupage' ? 'text-amber-400 bg-slate-900' : 'text-gray-400 hover:text-white'}`}
                  >
                    Groupages
                  </button>
                </div>
            ...
            {/* ASSISTANT CHATBOT FLOTTANT (Global, visible sur toutes les pages) */}
            <div className="fixed bottom-4 right-4 z-50">
              <button 
                ...
                className="w-12 h-12 rounded-full bg-gradient-to-r from-amber-400 to-amber-500 text-slate-950 flex items-center justify-center shadow-lg shadow-amber-500/20 hover:scale-110 active:scale-95 transition-transform"
